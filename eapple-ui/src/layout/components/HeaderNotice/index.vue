  ...
  <div>
    ...
    >
      <div class="notice-header">
        <span class="notice-title">平台通知</span>
      ...
      <div v-else-if="noticeList.length === 0" class="notice-empty"><i class="el-icon-inbox"></i><br>暂无通知</div>
      <div v-else>
        <div
  ...
  methods: {
    ...
    previewNotice(item) {
      if (!item.isRead) {
        ...
        if (idx !== -1) {
          this.$set(this.noticeList, idx, { ...item, isRead: true })
        }
    ...
    markAllRead() {
      const ids = this.noticeList.map(item => item.noticeId).join(',')
      if (!ids) {
        return
      }
      markNoticeReadAll(ids).catch(() => {})
      this.noticeList = this.noticeList.map(item => ({ ...item, isRead: true }))
      this.unreadCount = 0
    }
  }
}
