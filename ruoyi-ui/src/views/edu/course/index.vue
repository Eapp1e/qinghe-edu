    ...
    <el-table v-loading="loading" :data="courseList">
      ...
      <el-table-column label="状态" prop="status" width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status === '0' ? 'success' : 'info'">{{ scope.row.status === '0' ? '开放' : '关闭' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" min-width="320">
        <template slot-scope="scope">
          ...
          <el-button v-hasPermi="['edu:course:edit']" size="mini" type="text" @click="handleNotice(scope.row)">AI通知</el-button>
          <el-button v-hasPermi="['edu:course:edit']" size="mini" type="text" @click="handleSuggestion(scope.row)">AI建议</el-button>
          <el-button v-hasPermi="['edu:course:remove']" size="mini" type="text" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    handleEnroll(row) {
      ...
      if (this.$auth.hasRole('edu_parent')) {
        ...
        if (this.childrenList.length > 1) {
          this.$modal.msgError('当前账号关联了多个孩子，请先保留一个孩子档案或按需扩展页面选择功能')
          return
        }
    ...
    handleNotice(row) {
      generateCourseNotice(row.courseId).then(res => {
        this.$alert(res.data, 'AI生成通知', { dangerouslyUseHTMLString: false })
      })
    },
    handleSuggestion(row) {
      generateTeachingSuggestion(row.courseId).then(res => {
        this.$alert(res.data, 'AI教学建议', { dangerouslyUseHTMLString: false })
      })
    }
  }
}
